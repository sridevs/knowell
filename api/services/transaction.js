...
transaction.getBorrowedBookIdOfUser = async((userId) => {
    return await(knex.select('book_id').from('transaction').where({user_id: userId, return_date: null})).map( e => e.book_id);
});
...
transaction.borrowedBooksBy = async((userId) => {
    return await(knex.select('*').from('transaction')
        .join('book').join('title', {'title.id': 'book.title_id', 'book.id': 'transaction.book_id'})
...
transaction.isTitleBorrowedForUser = async((titleId, userId) => {
    return await(knex.select('*').from('transaction')
        ...
        .where({user_id: userId, title_id: titleId, return_date: null})).length != 0;
});

transaction.isTagNumberBorrowedByUser = async((tagNumber, userId) => {
    return await(knex.select('*').from('transaction')
            .join('book', {'book.id': 'transaction.book_id'})
            .where({user_id: userId, tag_number: tagNumber, return_date: null})).length != 0;
});
